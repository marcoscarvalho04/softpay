...
export class LoginContract implements Contract {
  ...
  validate(model: Merchant): boolean {
    ...
    if((model.cnpj == undefined || model.cnpj.length == 0) && (model.email == undefined || model.email.length == 0 ) ){
        this.errors.push('Email ou CNPJ devem ser presentes.')
    }
    if((model.password == undefined|| model.password.length == 0)){
        this.errors.push('Password é requerido.');
    }
